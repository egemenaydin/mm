...
data <- read.csv("yuzeysel_su_CYA_sta.csv", header = TRUE, check.names = FALSE)
data2 <- read.csv("yuzeysel_su_CYA_sta.csv", header = TRUE, check.names = FALSE)

data$Area <- NULL
data$Season <- NULL

data_polished <- data[, apply(data, 2, var, na.rm = TRUE) != 0]
data_polished[data_polished == 0] <- 0.05*(min(data_polished[data_polished>0],na.rm=TRUE))
...
PC <- predict(pca)
write.csv(PC, file = "PCA_YSCYA.csv")
x <- read.csv("PCA_YSCYA.csv", row.names = 1)
summary <- summary(pca)
loadings <- pca$rotation

write.csv(loadings, file="loadings_YSCYA.csv")
y <- read.csv("loadings_YSCYA.csv", row.names = 1)
...
g + geom_point(aes(shape = Area, color = Season), size = 5) + 
        theme_bw(base_size = 22) + xlab(x_lab) + 
        scale_shape_identity(labels = letters[]) +
        ylab(y_lab) + scale_colour_brewer(palette = "Set1")

dev.print(pdf, file = "PCA_silva.pdf", height=11, width=11)
...
dev.print(pdf, file = "loadings_silva_color.pdf", height=7, width=11)
