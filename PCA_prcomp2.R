...
data <- read.csv("silva.csv", header = TRUE, check.names = FALSE)
data2 <- read.csv("silva.csv", header = TRUE, check.names = FALSE)
...
Samples <- data2$Samples
pca <- prcomp(data_polished, center = TRUE, scale. = TRUE)

PC <- predict(pca)
write.csv(PC, file = "PCA_silva.csv")
x <- read.csv("PCA_silva.csv", row.names = 1)
summary <- summary(pca)
loadings <- pca$rotation

write.csv(loadings, file="loadings_silva.csv")
y <- read.csv("loadings_silva.csv", row.names = 1)
...
g + geom_point(aes(color = Samples), size = 5) + theme_bw(base_size = 22) + xlab(x_lab) + 
        ylab(y_lab) + scale_colour_brewer(palette = "Set1")

dev.print(pdf, file = "PCA_silva.pdf", height=11, width=11)

d <- ggplot(y, aes(PC1, PC2))
d + geom_point(size = 2, aes(color = PC1)) + theme_bw(base_size = 12) + xlab(x_lab) + ylab(y_lab) +
