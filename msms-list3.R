...
baseP1 <- filter(iso.dataP, grepl("\\[M\\]\\+", isotopes))
baseP2 <- filter(iso.dataP, grepl("\\[M\\]2\\+",isotopes))
baseP <- bind_rows(baseP1, baseP2)
...
data_polished <- baseAll_PCA[, apply(baseAll_PCA, 2, var, na.rm = TRUE) != 0]
...
x$Numune <- rownames(x)
z <- transform(x, Numune = ordered(Numune, fNames))
