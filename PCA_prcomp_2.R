...
data <- read.csv("StatsTable_planktonic.csv", header = TRUE, check.names = FALSE)
data2 <- read.csv("StatsTable_planktonic.csv", header = TRUE, check.names = FALSE)
...
Samples <- data2$label
pca <- prcomp(data_polished, center = TRUE, scale. = TRUE)
...
#color palette for two groups of sequential series and one different sample
my.palette <- c("#e5f5f9", "#ccece6", "#99d8c9", "#66c2a4", "#41ae76", "#238b45", "#005824", "#fde0dd", "#fcc5c0", "#fa9fb5", "#f768a1", "#dd3497", "#ae017e", "#7a0177", "#ffff99")

#color palette for two groups of sequential series
my.palette <- c("#e5f5f9", "#ccece6", "#99d8c9", "#66c2a4", "#41ae76", "#238b45", "#fde0dd", "#fcc5c0", "#fa9fb5", "#f768a1", "#dd3497", "#ae017e")

x_lab <- sprintf ('PC1 (%0.1f%%)', 100*pca$sdev[1]^2/sum(pca$sdev^2))
y_lab <- sprintf ('PC3 (%0.1f%%)', 100*pca$sdev[2]^2/sum(pca$sdev^2))

g <- ggplot(x, aes(PC1, PC3))
g + geom_point(aes(color = Samples), size = 5) + theme_bw(base_size = 22) + xlab(x_lab) + ylab(y_lab) + scale_colour_manual(values = my.palette)

dev.print(pdf, file = "PCA_identified_PC1PC3.pdf", height=11, width=11)
