...
#lists for plots
dataAll_PCA <- dataP[ , c("name", fNames[4:24])]
...
write.csv(dataAll_PCA, "all_ions_for_PCA_onlyMW.csv")
...
Samples <- fSamples[4:24]
pca <- prcomp(log.data, center = TRUE, scale. = TRUE)

PC <- predict(pca)
write.csv(PC, file = "PCA_MW_log.csv")
x <- read.csv("PCA_MW_log.csv", row.names = 1)
summary <- summary(pca)
loadings <- pca$rotation

write.csv(loadings, file="loadings_MW_log.csv")
y <- read.csv("loadings_MW_log.csv", row.names = 1)
...
dev.print(pdf, file = "PCA_MW_log.pdf", height=11, width=11)

d <- ggplot(y, aes(PC1, PC2))
d + geom_point(size = 2) + theme_bw(base_size = 12) + xlab(x_lab) + ylab(y_lab)

dev.print(pdf, file = "loadings_MW_log.pdf", height=7, width=11)
