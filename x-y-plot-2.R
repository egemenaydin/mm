...
ggplot(data, aes(x = Sample, y = Abundance)) + 
        ...
        geom_point(size = 5, color = "black", shape = 15) + 
        geom_point(size = 3, aes(color = Category), shape = 3) +
        guides(colour = guide_legend(override.aes = list(shape = 15, size = 5))) +
        theme_bw(base_size =16, base_family = "Times") + 
        geom_errorbar(aes(ymax = Abundance + Sdev, ymin = Abundance - Sdev), width = 0.25) + 
        theme(axis.text.x = element_text(angle = 90, hjust = -0.25, vjust = 0.5)) + 
        scale_x_discrete(limits = (data$Sample)[order(data$Category)]) + 
        annotate("text", x = 0.75, y = 40000, label = "Not detected", angle = 90, size = 7, family = "Times") +
